...
//echo "<br>imp_array".print_r($imp) ;
    ...
    foreach ($imp as $keys => $val) {
       ...
       $mid=floor(($low+$high)/2);
        ...
        if ($pl_set >= $imp[$keys][0] &&  $pl_set <= $imp[$keys][1]) {
        ...
        while ($final[$pl_set] != '' && $pl_set <= $bracketSize && ($byset[$flag][1] <= $byset[$flag][0])) {
            $kcount = 0;
            //  echo "<br> in while x== ".$x;
            //   echo "<br>final[pl_set]['coach']  ::  ". $final[$pl_set]['coach'];
            echo "<br>before while pl set ==" . $pl_set;
            echo "<br>before while  flag==" . $flag . "   x==" . $x;

            // echo "<br>z" . $imp[$flag][1];
            // echo "<br>x" . $imp[$flag][0];
            // print_r($final);
            // echo "<br> " . $i;
            ...
            if ($final[$pl_set]['coach'] == $sub_val['coach']) {
                echo "<br>in if";
                $pl_set = (($flag * (32 / $x)) + (($flag + 1) * (32 / $x))) / 2;
                //$nw_pl_set=   ($pl_set+ ($pl_set + ($pl_set/$y)))/2;
                //echo "<br>new_plset =  ".$nw_pl_set;
                $pl_set++;
                $x = ($x >= 16) ? 1 : ($x * 2);

               //  echo "<br>pl set in half=".$pl_set;


             $y=$y*2;
            } else {
                echo "<br>in else";
                //  echo "<br>pl set final elese==".$pl_set;
                $pl_set++;
            }
            // for ($i = $xy; $i <= $yz; $i++) {
                //     if ($final[$i]['player_name'] == 'Bye') {
                    //         $kcount++;
        //     }
        // }
        // if ( $kcount <= ($byset[$flag][0]) && $sub_val['coach'] = 'AAAA') {
            //     echo "exit_pl_set".$pl_set;
            //     // exit;
            //     // break;
            // }
            echo "<br>kcount = " . $kcount;
            echo "<br>in while pl set ==" . $pl_set;
            echo "<br>in while flag==" . $flag . "   x==" . $x;
    }
    echo "<pre>byearray";
print_r($byset);
