...
if (isset($_POST['display'])) {
    ...
    // $cntb = "SELECT COUNT(belt)FROM karate_form_data where $query";
    // //echo $cntb;
    // $cb = mysqli_query($mysqli, $cntb);
    // $c2 = mysqli_fetch_assoc($cb);
    //     echo "<td>".$belt ."=";
    //     $c1;
    //     // $cnt[0];
    //     print_r($c2);
    ...
    // $cntkata = "SELECT COUNT(kata)FROM karate_form_data where $query";
    // //echo $cntkata;
    // $ck = mysqli_query($mysqli, $cntkata);
    // $c3 = mysqli_fetch_assoc($ck);
    //     echo "<td> Kata=".$kata."(";
    //    // $c1;
    //     // $cnt[0];
    //     print_r($c3);
    ...
    if ($gen && $result !== false && $row > 0) {

        $cntg = "SELECT count(case when gender='$gen' then 1 end) as f , count(case when kata='yes' then 1 end) as kata_y, count(case when kata='no' then 1 end) as kata_n , count(case when kumite='yes' then 1 end) as kum_y , count(case when kumite='no' then 1 end) as kum_n FROM karate_form_data where $query";
        // $cntg = "SELECT count( count(case when gender='Male' then 1 end) as m,count(case when gender='Female' then 1 end) as f) FROM karate_form_data where $sql";
        // $cntg = "SELECT count(case when gender='Female' then 1 end) as f , count(case when kumite='yes' then 1 end) as kumite_y, count(case when kumite='no' then 1 end) as kumite_n FROM karate_form_data where $query";
        ...
        echo "<tr>";
        echo "<td>$gen</td>";
        echo "<td>$c1[kata_y]</td>";
        echo "<td>$c1[kum_y]</td>";
        echo "</tr>";
        echo "</table>";
    }

    if (empty($gen) && $result != FALSE) {
        $cntgm = "SELECT count(case when gender='male' then 1 end) as m , count(case when kata='yes' then 1 end) as kata_y, count(case when kata='no' then 1 end) as kata_n , count(case when kumite='yes' then 1 end) as kum_y , count(case when kumite='no' then 1 end) as kum_n FROM karate_form_data where  gender = 'male' and  $query ";
        $cntgf = "SELECT count(case when gender='Female' then 1 end) as f , count(case when kata='yes' then 1 end) as kata_y, count(case when kata='no' then 1 end) as kata_n , count(case when kumite='yes' then 1 end) as kum_y ,  count(case when kumite='no' then 1 end) as kum_n FROM karate_form_data where gender = 'female'  and  $query ";
...
if ($res > 0) {
    ...
    echo "<h2>Sorry,<br>No data found for  this match try for different match.</h2>";
}
...
function getPlayerNames($bracket, $player, $data)
{
    ...
    //echo "<br>Number of Groups".$g_count;
    ...
    //                 // echo "<br>in array";
    //                 if(!in_array($search_key,$repeated));
    ...
    if (count($player) > 2) {
        ...
        foreach ($player as $p) {
            ...
            // Check repeated player or not
            if ( array_search($p[0], array_column($pl_name, 0)) !== false && $p[0] !== 'Bye' ) {

                //echo "<br>in Pl_name ".$player[array_search($p[0], array_column($player, 0))][0];
                $i++;
                //echo " ".$i;
            } 
            else
        ...
        //echo "<br>Number of player in pl_name" . $i;
    }
        ...
        foreach ($pl_name as $search_key => $search_array) {
            ...
            if ($search_array[0] == $current_array[0] && $pl_name[$search_key][0] != 'Bye') {
                ...
                if ($search_key != $current_key) {
                    ...
                    // echo "hello dupes";
                     array_push($repeated, $pl_name[$search_key][0]);
                     array_push($repeated_index, $search_key);
    ...
    if (count($new) > 0) {
        ...
        echo "New";
        echo "<pre>";
        echo "pl_name". print_r($pl_name);
        print_r($new);
        $repeated = array_unique($repeated);
        print_r($repeated);
        print_r($repeated_index);
        
        foreach($repeated as $k => $v )
        {
            foreach($repeated_index as $key => $value)
            {
                if($k == $key)
                {
                    $repeated[$k] = $repeated_index[$key];
                }
            }
        }
        print_r($repeated);
        ...
        foreach($repeated as  $v )
        {
            foreach($new as $key => $value)
            {
                ...
                {
                    echo "<br>re_key".$v;
                    echo "<br>new_key".$value;
                    $pl_name[$v][0] = $player[$value][0] ;
                    unset($new[$key]);
                    $flag = 1;
                }
            }
       ...
       // echo "<pre>";
        ...
        foreach ($newbracket as $k) {
            ...
            echo "<tr><td colspan = '2' >Pool-" . $alpha . "</td></tr>";
            echo "<tr><td style='padding:0px; border: 0 none;'><table><tr>";
            echo "<th>No</th>";
            echo "<th> Corner </th></tr>";
    ...
    echo"</tr>";
    echo"<tr>";
    echo"<td>1.___________________________________________ </td>";
    echo"<td>_____________________________________________ </td>";
    echo"</tr>";
    echo"<tr>";
    echo"<td>2.___________________________________________ </td>";
    echo"<td>_____________________________________________ </td>";
    echo"</tr>";

    echo"<tr>";
    echo"<td>3.___________________________________________ </td>";
    echo"<td>_____________________________________________ </td>";
    echo "</tr>";

    echo "<tr>";
    echo"<td>4.___________________________________________ </td>";
    echo"<td>_____________________________________________ </td>";
    echo"</tr>";
    echo"</table>";
    echo"</body>";
